...
def propogation(op2):
    ...
    if(_int_type >= 0): 
        ...
        wp_arI = array('d', [0]*wfr2.mesh.ne*wfr2.mesh.nx*wfr2.mesh.ny)
        print ("@@@")
        srwl.CalcIntFromElecField(wp_arI, wfr2, 0, 4, 3, wfr2.mesh.eStart, wfr2.mesh.xStart, wfr2.mesh.yStart)
        if(True):
            sValUnitName = 'ph/s/.1%bw/mm^2' 

            print('Saving Propagation Results ... ', end='')
            
            srwl_uti_save_intens_ascii(wp_arI, wfr2.mesh,"Direct2mask_Wfr_phase.dat", 0, ['Photon Energy', 'Horizontal Position', 'Vertical Position', ''], _arUnits=['eV', 'm', 'm', sValUnitName])
        return wp_arI           
...
def main():
    ...
    global xRatio
    global yRatio
    xRatio =7.32e-7 /((srwl_bl.wfr.mesh.xFin - srwl_bl.wfr.mesh.xStart)/2048)  #pixel_size * pixel_num / (srwl_bl.wfr.mesh.xFin - srwl_bl.wfr.mesh.xStart)
    yRatio =7.32e-7 /((srwl_bl.wfr.mesh.xFin - srwl_bl.wfr.mesh.xStart)/2048) #pixel_size * pixel_num / (srwl_bl.wfr.mesh.yFin - srwl_bl.wfr.mesh.yStart)
    ...
    srwl_uti_save_intens_ascii(maskWfr_phase, wfr_phase.mesh, os.path.join(os.getcwd(), "Direct2mask_Wfr_phase.dat"), 0,
